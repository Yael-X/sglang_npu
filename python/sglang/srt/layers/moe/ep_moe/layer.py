...
class DeepEPMoE(EPMoE):
    ...
    def forward_npu_normal(
        ...
        print(f"=========rank: {dist.get_rank()}, hidden_states: {hidden_states.dtype}, {hidden_states.shape}, w13_weight: {self.w13_weight.dtype}, {self.w13_weight.shape}")
        print(f"=========rank: {dist.get_rank()}, self.w13_weight_scale: {self.w13_weight_scale.dtype}, {self.w13_weight_scale.shape}")
        print(f"=========rank: {dist.get_rank()}, group_list: {group_list}")

        import torch_npu

        hidden_states, per_token_scale = torch_npu.npu_dynamic_quant(hidden_states)
        print(f"=========rank: {dist.get_rank()}, per_token_scale: {per_token_scale.shape}, {per_token_scale.dtype}")
